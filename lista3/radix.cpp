...
int main(int argc, char* argv[])
{
    ...
    else if (mode == 2)
    {
        while (!src.empty())
        {
            ...
            src.pop_back();
            int s = src.back();
            auto dist = g->dijkstra_radix_p2p(s, dest);
            src.pop_back();
            my_file << "radix: source: " << s << " dest: " << dest << "distance: " << dist << std::endl;
        }
    }
